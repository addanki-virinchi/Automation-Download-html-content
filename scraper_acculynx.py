...
BASE_URL = "https://www.acculynx.com"
...
class TopNavParser(HTMLParser):
    ...
    def _should_enter_header(self, tag, attrs) -> bool:
        if tag != "header":
            return False
        if attrs.get("id") == "page-header":
            return True
        class_list = self._class_list(attrs)
        return "l-header" in class_list

    def _should_enter_footer(self, tag, attrs) -> bool:
        if tag != "footer":
            return False
        if attrs.get("id") == "page-footer":
            return True
        class_list = self._class_list(attrs)
        return "l-footer" in class_list
...
def is_acculynx_domain(url: str) -> bool:
    parsed = urlparse(url)
    if not parsed.netloc:
        return False
    return parsed.netloc.lower().endswith("acculynx.com")
...
def should_skip_link(text: str, href: str, attrs: dict) -> bool:
    ...
    if any(cls in class_list for cls in ("w-image-h", "w-socials-item-link")):
        return True
    return False
...
def extract_top_nav_links(html_text: str):
    ...
    for text, href, attrs in parser.collected:
        ...
        if not is_acculynx_domain(full_url):
            continue
...
def main() -> None:
    with open("acculynx.html", "r", encoding="utf-8") as handle:
        html_text = handle.read()

    links = extract_top_nav_links(html_text)

    with open("navigation_links_acculynx.csv", "w", newline="", encoding="utf-8") as handle:
